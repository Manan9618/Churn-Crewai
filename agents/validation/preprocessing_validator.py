...
def create_preprocessing_validator(llm) -> Agent:
    return Agent(
        ...
        goal=(
            ...
            "values, applied proper encoding to categorical variables, scaled numerical "
            "features, and produced a clean train/test split with no data leakage."
        ),
        backstory=(
            "You are a meticulous QA engineer for ML pipelines. You check that after "
            "preprocessing: no NaNs remain, all categorical columns are numeric, feature "
            "ranges are standardised, and the target distribution is maintained across "
